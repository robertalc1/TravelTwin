...
|---|---|---|---|
| 2026-05-02 | fix | **Favorites no longer fail with "Couldn't update favorite — try again".** Root cause: schema mismatch — the API and components wrote `city_name` / `city_data`, but the actual `public.favorites` table has `item_type` (CHECK in `'city' \| 'attraction' \| 'hotel'`), `item_id`, `item_name`, `item_data`. RLS policies on the table were already correct (SELECT/INSERT/DELETE all check `auth.uid() = user_id`), and the `on_auth_user_created` trigger auto-creates `profiles` rows so the FK is satisfied. Rewrote `/api/favorites` (GET/POST/DELETE) to use the real columns; updated `Favorite` type, `FavoriteButton`, the `/explore` toggle, the `/favorites` page, and the profile FavoritesTab. `FavoriteButton` now surfaces the API's actual error message in the toast, and shows "Added to favorites ✓" on success. | _this commit_ |
