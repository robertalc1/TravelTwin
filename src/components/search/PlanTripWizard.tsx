...
const defaultDep = new Date(today.getTime() + 14 * 24 * 60 * 60 * 1000)
  .toISOString()
  .split("T")[0];
const defaultRet = new Date(today.getTime() + 21 * 24 * 60 * 60 * 1000)
  .toISOString()
  .split("T")[0];

export function PlanTripWizard({ isOpen, onClose }: PlanTripWizardProps) {
  ...
  function toggleStyle(id: string) {
    set(
      "travelStyles",
      state.travelStyles.includes(id)
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!data.packages?.length)
        throw new Error(
          "No trips found for your criteria. Try adjusting your budget or dates."
        );
      ...
      <div className="fixed inset-0 z-[9999] bg-gradient-to-br from-secondary-500 via-secondary-600 to-primary-600 flex items-center justify-center">
        <div className="w-full max-w-lg px-6 text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">
            Your AI travel agent is working...
          </h2>
          <p className="text-white/70 mb-10">
            We&apos;re finding your perfect trip
          </p>
          <div className="space-y-3 mb-10 text-left">
            {LOADING_STEPS.map((s, i) => (
              ...
              >
                ...
                <span
                  className={`text-sm ${i <= loadingStep ? "text-white" : "text-white/50"}`}
                >
                  {s.text}
                </span>
              </div>
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex flex-col bg-neutral-50 dark:bg-background">
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-neutral-200 dark:border-border-default bg-white dark:bg-surface shrink-0">
        ...
        <button
          onClick={handleClose}
          className="flex h-9 w-9 items-center justify-center rounded-full hover:bg-neutral-100 dark:hover:bg-surface-elevated transition-colors"
        >
      ...
      {/* Progress bar */}
      <div className="h-1 bg-neutral-200 dark:bg-border-default shrink-0">
        <div
          ...
          <AnimatePresence custom={direction} mode="wait">
            ...
            {step === 0 && (
              ...
              >
                ...
                <div className="bg-white dark:bg-surface rounded-2xl shadow-lg p-8 max-w-md mx-auto">
                  <label className="block text-sm font-bold text-text-primary mb-3 text-left">
                    <MapPin className="h-4 w-4 inline mr-2 text-primary-500" />
                    Departing from
                  </label>
                  ...
                  {state.originDisplay && (
                    <div className="mt-4 flex items-center gap-2 text-sm text-green-600 dark:text-green-400">
                      ...
                      <span>
                        Departing from{" "}
                        <strong>{state.originDisplay}</strong>
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-white dark:bg-surface rounded-2xl shadow-lg p-8 space-y-6">
                  ...
                  <div className="text-left">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const ret = new Date(
                          dep.getTime() + state.nights * 24 * 60 * 60 * 1000
                        );
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-text-muted mt-3">
                      Return:{" "}
                      {new Date(state.returnDate).toLocaleDateString("en-GB", {
                        weekday: "short",
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                  ...
                  <div className="text-left">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      {(
                        [
                          { label: "Adults", field: "adults" as const },
                          { label: "Children", field: "children" as const },
                        ] as const
                      ).map(({ label, field }) => (
                        <div
                          key={field}
                        ...
                        >
                          ...
                          <button
                            onClick={() =>
                              set(field, Math.min(9, state[field] + 1))
                            }
                            className="h-9 w-9 rounded-full border border-neutral-200 flex items-center justify-center text-lg font-medium hover:bg-neutral-100 transition-colors"
                          >
                            +
                    ...
                    {TRAVEL_STYLES.map((style) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {selected && (
                            <CheckCircle2 className="h-4 w-4 text-primary-500 ml-auto shrink-0" />
                          )}
                        </button>
                      );
                    })}
                ...
                <div className="bg-white dark:bg-surface rounded-2xl shadow-lg p-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {PRIORITY_OPTIONS.map((p) => {
                      ...
                      const disabled =
                        !selected && state.priorities.length >= 3;
                      return (
                        <button
                          key={p.id}
