...
function HotelsSearchContent() {
  ...
  const [warning, setWarning] = useState<string | null>(
    datesMissing
      ? (isRo
          ? "Lipsesc datele de căutare. Deschide pagina dintr-o călătorie planificată."
  ...
  useEffect(() => {
    ...
    fetch(`/api/hotels/search?${qs.toString()}`)
      ...
      .catch(() => {
        if (!cancelled) setWarning(isRo ? "Nu am putut încărca hotelurile." : "Failed to load hotels.");
      })
      .finally(() => {
        ...
        <div className="mx-auto max-w-[1400px] px-4 lg:px-8 py-3 flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-base sm:text-lg font-bold text-text-primary truncate">
              {isRo ? `Cazări în ${cityName}` : `Find stays in ${cityName}`}
            </h1>
            <p className="text-xs text-text-muted">
              {hotels.length} {isRo ? (hotels.length === 1 ? "rezultat" : "rezultate") : (hotels.length === 1 ? "result" : "results")}
        ...
        >
          <div className="rounded-2xl border border-neutral-200 dark:border-border-default bg-white dark:bg-surface p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold text-text-primary">{isRo ? "Filtre" : "Filters"}</h2>
              <button
                type="button"
            ...
            <section className="mb-5">
              ...
              >
                ...
                <option value="rating">{isRo ? "Cele mai multe stele" : "Highest stars"}</option>
              </select>
            </section>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-text-secondary">
                {hotels.length === 0
                  ? (isRo ? "Niciun hotel disponibil pentru aceste date." : "No hotels available for these dates.")
                  : (isRo ? "Niciun hotel nu corespunde filtrelor." : "No hotels match your filters.")}
              </p>
              {hotels.length > 0 && (
                <button
                  type="button"
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-neutral-50 dark:bg-background">
      <div className="text-center">
        ...
        <p className="text-text-secondary">
          {isRo ? "Se încarcă hotelurile..." : "Loading hotels..."}
        </p>
      </div>
    </div>
  );
...
export default function HotelsSearchPage() {
  return (
    <Suspense fallback={<HotelsSearchFallback />}>
      <HotelsSearchContent />
    </Suspense>
  );
}
